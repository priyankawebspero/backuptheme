...
(function ($) {
  ...
  var ella = {
    ...
    init: function () {
      ...
      this.recentlyViewAddToCart();
      this.initGroupedAddToCart();
      this.initSliderFeaturedProducts();
      this.initSliderFeaturedProductsHome();
      this.initSliderFeaturedProductsNew();      
      this.addEventLookbookModal();
      this.initPoliciesSlider();
      this.initCountdown();
      this.initCountdownNormal();
      this.selectCallBackFunction();
     ...
     CRMUserVaildation: function () {
       $('#crm_customers_form').submit(function(e){
          e.preventDefault();
          var form = $(this);
          var email = $(this).find('#customers_email').val();
          var phone = $(this).find('#customers_phone_number').val();
          var discount_code = $(this).find('#user_validation').val();
          var redirect_url = $(this).find('#url').val();
          var emailcheck = ValidateEmail(email);            
          if (!emailcheck) {
            $("#crm_customers_form").find('.message').remove();
            return $(form).append('<div class="message"><p class="error">This email is not vaild!</p><div>');
          }
          $('.pop-out_main_cd div#loader').show();
          var AppUrl = "https://order.plumgoodness.com/customers/"+email
          $.ajax({
            type: 'POST',
            url: AppUrl,
            dataType: 'json', 
            success: function (data) {
              $('body').addClass('customer_found');
              $("#crm_customers_form").find('.message').remove();
              $(form).trigger("reset");
              $('#crm_customers_v.pop-out_main_cd div#loader').hide();
              $(form).append('<div class="message"><p>Oops this offer is only valid for new users!</p><div>');
              // if (!redirect_url) {redirect_url = "/";}
              // window.location.href = 'https://plumgoodness.com/'
              $('.crm_popup_top').hide();
              $(form).find('input').hide();
             // $(form).find('.message').append('<a href="/" class="btn user_not_vaild_button">back To homepage</a>');
              // set and get cokies for popup home page
            ...
            error: function(error){
               $("#crm_customers_form").find('.message').remove();
                ...
                $('.crm_popup_top').hide();
                $(form).find('input').hide();
               $(form).append('<div class="message"><p>Congratulations!! Use Code : <b>FLASH10</b> to get extra 10% Off</p><div>');
                clevertap.event.push("CRM_PM New Users", {
                	email: email,
                	phone: phone
             	 });
               insertToSheetApi(email,phone);
                            function setCookie(cname,cvalue) {
                  var d = new Date();
                  d.setTime(d.getTime() + (60*60*1000));
                  var expires = "expires=" + d.toGMTString();
      ...
      $('#crm_customers_v .close').click(function(){
                      function setCookie(cname,cvalue) {
                  var d = new Date();
                  d.setTime(d.getTime() + (60*60*1000));
                  var expires = "expires=" + d.toGMTString();
         ...
         setCookie("customer_email", "halo@plumgoodness.com"); 
        $('#crm_customers_form .pop-out_main_cd').removeClass('user_not_vaild').addClass('user_vaild');
        $('#crm_customers_form .pop-out_main_cd').remove('.message');
         // console.log('close');
      });                     
    },
	UserVaildation: function () {
       $('#customers_form').submit(function(e){
          ...
          var emailcheck = ValidateEmail(email);            
          if (!emailcheck) {
            $("#customers_form").find('.message').remove();
            return $(form).append('<div class="message"><p class="error">This email is not vaild!</p><div>');
          }
          $('.pop-out_main_cd div#loader').show();
          var AppUrl = "https://order.plumgoodness.com/customers/"+email
          $.ajax({
            ...
            success: function (data) {
              ...
              $("#customers_form").find('.message').remove();
              $(form).trigger("reset");
              // $('body').find('.pop-out_main_cd').removeClass('user_vaild').addClass('user_not_vaild');
              $('.pop-out_main_cd div#loader').hide();
              if ($('body').hasClass('template-index')) {
                $('.cs_popup_em_cs .popup_top').hide();
                $('.cs_popup_em_cs input').hide();
              }
              window.location.href = 'https://plumgoodness.com/'
             // $(form).append('<div class="message"><p>Oops this offer is only valid for new users!</p><div>');
              // if (!redirect_url) {redirect_url = "/";}
            ...
            error: function(error){
               $("#customers_form").find('.message').remove();
                ...
                //$(form).append('<div class="message"><p>Congratulations!! Use Code : <b>FLASH10</b> to get extra 10% Off</p><div>');
                $('.pop-out_main_cd').hide();
                // if (insertedData['status'] == "Email already exist") {
                   
                //     $(form).append('<div class="message"><p>Oops this offer is only valid for new users you already use this email!</p><div>');
                //     return;
                // }else{
                //  $("#customers_form").find('.message').remove();
                //   $('.pop-out_main_cd div#loader').hide();
    ...
    showHideMenuMobile: function () {
      if (iconNav.length && iconNav.is(':visible')) {
        iconNav.off('click.showMenuMobile').on('click.showMenuMobile', function (e) {
          ...
          $('.site-nav').find('.sub-menu-open-1').addClass('sub-menu-open');
        })
      };
    },
    ...
    }, initSliderFeaturedProductsHome: function () {
      var featuredProduct = $('[data-featured-products-home]');
    ...
    brandsStyle3: function() {
      var brandsSlider = $('[data-brands-slider3]');
    ...
    checkItemsInDropdownCart: function () {
      var cartNoItems = dropdownCart.find('.no-items'),
          ...
          cartsubtotal = dropdownCart.find('.actions');

      if (miniProductList.children().length) {
        cartHasItems.show();
        cartNoItems.hide();
         cartsubtotal.css({'visibility' : 'visible', 'opacity' : '1'});
      } else {
        cartHasItems.hide();
        cartsubtotal.css({'visibility' : 'hidden', 'opacity' : '0'});
        cartNoItems.show();
      };
    },
...
//         $('.special_offer_p').html('<div class="special_offer"> Pick plums worth '+Shopify.formatMoney(actu_price, window.money_format)+' more & get <span>FREE vanilla vibes shower cream</span>  <img src="https://cdn.shopify.com/s/files/1/0390/2985/files/like_1.png?v=1589371393"> </div>');      
      ...
      if (cart.item_count > 0) {
        ...
        for (var i = 0; i < cart.items.length; i++) {
          ...
          template += '<a href="javascript:void(0)" title="{{ "cart.general.remove" | t }}" class="btn-remove">remove</a></div></div></li>';
          var item = template;
          $.ajax({ 
...
//       });
    ...
    frequentlyBrougtTogether: function(){
      $("#frequently-brought-button").click(function(){
        ...
        $(".add-to-cart-wrapper input").each(function(){
          if($(this).is(':checked')){
            ...
            jQuery.ajax({
              ...
              url: '/cart/add.js',
              dataType: 'json',
              data: 'quantity=' + 1 + '&id=' + productVariant,
              async:false,
              beforeSend: function () {
                ella.showLoading();
              },
    ...
    recentlyViewAddToCart: function(){
      $("body").on("click",".recently-viewed-products .add-to-cart-btn-recently",function(e){
        e.preventDefault();
        e.stopPropagation();
        var variantID = $(this).closest("form").find("[name='id']").val();
        //             alert(variantID)
        jQuery.ajax({
          type: 'POST',
          url: '/cart/add.js',
    ...
    initBlogPostSlidernew: function() {
...
initBlogPostSlider: function() {
    ...
    initToggleSubMenuMobile: function () {
      ...
      iconDropdown.off('click.dropdownMenu').on('click.dropdownMenu', function (e) {
        ...
        if (curMenu.hasClass('sub-menu-open')) {
          curParent.removeClass('active-menu');
          curMenu.removeClass('sub-menu-open');
        } else {
          curParent.addClass('active-menu');
    ...
    initNewsLetterPopup: function () {
      if (window.newsletter_popup) {
        ...
        closeWindow.click(function (e) {
          ...
          ella.closeEmailModalWindow(newsletterWrapper);
        });
    ...
    initToggleWidgetTitleSidebarFilter: function () {
      ...
      body.off('click.toggleWidgetContent').on('click.toggleWidgetContent', widgetTitleSlt, function () {
        ...
        if($(this).hasClass('open')){
          $(widgetTitleSlt).removeClass('open');
    ...
    initAddToCart: function () {
      var btnAddToCartSlt = '[data-btn-addToCart]';
       if(localStorage.getItem("currentPincode")!=null && localStorage.getItem("currentPincode") != undefined){
        jQuery("#pincode_input").val(localStorage.getItem("currentPincode"));
        $("#check-delivery-submit").text("Edit");
        $('input.pincode_input').attr('readonly', true); 
        jQuery(".availability-text").html("<span style='color:green; padding: 8px 0 0;font-weight: 500;'>Shipping Available! There will be a delay in shipping of your order owing to the COVID situation.</span>").fadeIn(100);
      }


      // Pincode Checker
      var foo = window.pincode;
      
      $('#check-delivery-submit').click(function(e) { 
        var buttonText = $(this).text();
        console.log("foo---",foo); 
        if(buttonText == "Check"){
          var MyZipCode=jQuery("#pincode_input").val();
          var cod = foo.indexOf(MyZipCode);

 
           if(MyZipCode == 0) {
            jQuery(".availability-text").empty();
            $("#pincode_input").css("");
            jQuery(".availability-text").html("<span style='color:red; padding: 8px 0 0;'>Please enter valid pincode.</span>").fadeIn(100);
            //                $(".add-to-cart").css("opacity","0.1");
            e.preventDefault();
          }
          else if(MyZipCode.length < 6) {
            jQuery(".availability-text").html("<span style='color:red; padding: 8px 0 0;'>Invalid pincode</span>").fadeIn(100);
            $("#pincode_input").css("border","none");
            e.preventDefault();
          }
          else if (cod != -1 ) {
            jQuery(".availability-text").html("<span style='color:green; padding: 8px 0 0;font-weight: 500;'>Shipping Available!</span>").fadeIn(100);
            ...
            jQuery(".availability-text").html("<span style='color:red; padding: 8px 0 0;'>Shipping is not available.</span>").fadeIn(100);
            $("#pincode_input").css("border","none");
            //                $(".add-to-cart").css("opacity","0.1");
          ...
          $("#check-delivery-submit").text("Check");
          localStorage.removeItem("currentPincode");
          jQuery("#pincode_input").val("");
          $('input.pincode_input').attr('readonly', false); 
        }
      });
    ...
    selectCallBackFunction:function (){
    ...
    changeSwatch: function (swatchSlt) {
     ...
     productPageInitProductTabs: function () {
      var listTabs = $('.tabs__product-page'),
        ...
        var TabContentlength = $('.tab-content.active').html().length;
        if (TabContentlength <= 2300){
          console.log('TabContentlength', TabContentlength);
          $('.tab-content.active .read_more').hide();
          $('.tab-content.active .read_less').hide();    
        }
      ...
      doc.on('click.triggerTabsReviews', sprBadge, function () {
        var reviewScrollTop = $(".three-sections .review").offset().top - 100;
        console.log(reviewScrollTop);
        $('html,body').animate({
    ...
    initStickyAddToCart: function() {
      ...
      if(blockSticky.length) {
        ...
        $('div.cart-icon > a').click(function() {
          Shopify.getCart(function(cart) {
            ella.doUpdateDropdownCart(cart);
          });
        });
...
function setDataTolocal(email , phone){
  if (!email) {return "email not vaild"}
  localStorage.setItem("customer_email",email);
  var customer_email = localStorage.getItem("customer_email");
  insertToSheetApi(email,phone);
}

function insertToSheetApi(email,phone){
    if (!email) {return "email not vaild"}
    var AppUrl = "https://order.plumgoodness.com/updateNewCustomerSheet/"+email+"/"+phone;
   $.ajax({
        type: 'POST',
        url: AppUrl,
        dataType: 'json', 
